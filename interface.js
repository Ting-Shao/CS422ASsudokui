...
$(function () {
  ...
  // Set the selected number to the 'editor'.
  setcurnumber(-1);
...
$(document).on('mousedown', 'td.sudoku-cell', function (ev) {
  ...
  if (num == -2) {
    ...
    var snNum = parseInt($(this).attr('id').substr(2));
    var el = $("#sn" + snNum);
    console.log(el.text())
    if (state.answer[pos] != null) {
      ...
      var save = function () {
        ...
        if (inputValue === parseInt(inputValue, 10) && inputValue < 4 && inputValue >= 0) {
          state.answer[pos] = inputValue;
          state.work[pos] = 0;
          ...
          $(document).trigger('log', ['invalidInput', { inputValue: inputValue }]);
          showpopup('#invalidInput');
...
/* $(document).on('click', '#auxiliarybutton', function (ev) {
  console.log($('#auxiliarybutton').prop('value'))
  if ($('#auxiliarybutton').prop('value') == 'Auxiliary mode : On') {
    $('#auxiliarybutton').prop('value', 'Auxiliary mode : Off');
    $('#auxiliarybutton').prop('style', 'background-color : #707070');
    $(document).trigger('log', ['auxiliary mode', { mode: "Off" }]);
  } else {
    $('#auxiliarybutton').prop('value', 'Auxiliary mode : On');
    $('#auxiliarybutton').prop('style', 'background-color : Aqua');
    $(document).trigger('log', ['auxiliary mode', { mode: "On" }]);
  }
}); */
...
function numberkeyhtml() {
  ...
  result += '<tr><td class=numberkey-cell id=nk0>' +
    ...
    '&#xf12d;</div></td></tr>';
  result += '<tr><td class=numberkey-cell id=nk-1>' +
    '<div class="edit nk-1">' +
    handglyph("edit") + '</div></td></tr>';
  result += '</table>';
  return result;
}
